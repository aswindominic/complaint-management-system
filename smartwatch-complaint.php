...
<head>
  <meta charset="UTF-8">
  <title>Smartwatch Complaint Form</title>
  <link rel="stylesheet" href="smartwatch-style.css">
</head>
<body>
  <div class="container">
    <h2>Smartwatch Complaint Registration</h2>

    <!-- Technician Availability -->
    <p style="text-align: center; color: green; font-weight: bold;">
      🛠 Available Technicians for Smart Watch: <?php echo htmlspecialchars($availableTechs); ?>
    </p>

    <form class="complaint-form" id="complaintForm" action="submit_complaint.php" method="POST" enctype="multipart/form-data">
      <input type="hidden" name="device_type" value="Smart Watch" />

      <div class="input-group">
        <label for="preset">Choose a common complaint (optional)</label>
        <select id="preset" name="complaint_type">
          <option value="">-- Select an issue --</option>
          <option value="Battery draining quickly">Battery draining quickly</option>
          <option value="Touchscreen not responding">Touchscreen not responding</option>
          <option value="Bluetooth not pairing">Bluetooth not pairing</option>
          <option value="Inaccurate heart rate tracking">Inaccurate heart rate tracking</option>
          <option value="App sync issues">App sync issues</option>
          <option value="Device overheating">Device overheating</option>
          <option value="Strap or casing damage">Strap or casing damage</option>
